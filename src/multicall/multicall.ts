...
type CallParams = Pick<CallByGasLimitParams, 'chainId' | 'client' | 'gasBuffer' | 'blockConflictTolerance' | 'dropUnexecutedCalls' | 'signal'>;
...
async function call(calls: MulticallRequestWithGas[], params: CallParams): Promise<CallResult> {
    const { chainId, client, gasBuffer = getDefaultGasBuffer(chainId), blockConflictTolerance = getBlockConflictTolerance(chainId), dropUnexecutedCalls = false, signal } = params;
    if (!calls.length) {
        return {
            results: [],
            blockNumber: 0n,
    ...
    console.warn(
        `Gas limit reached. Total num of ${calls.length} calls. First ${lastSuccessIndex + 1} calls executed. The remaining ${calls.length -
            lastSuccessIndex -
            1} calls are not executed. Pls try adjust the gas limit per call.`,
    );
    ...
    const { results: remainingResults, blockNumber: nextBlockNumber } = await call(calls.slice(lastSuccessIndex + 1), params);
