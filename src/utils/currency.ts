import { Currency, CurrencyAmount, Token, ChainId, WETH9, Ether } from '@real-wagmi/sdk'
...
export function unwrappedToken(token: Currency): Currency {
  ...
  if (token.equals(WETH9[token.chainId as ChainId])) return Ether.onChain(token.chainId)
  return token
}
