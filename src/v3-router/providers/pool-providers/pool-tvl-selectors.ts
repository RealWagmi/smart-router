import { ChainId, Currency, Token, Ether } from '@real-wagmi/sdk';
import { Address } from 'viem';

import { PoolSelectorConfig, V3PoolWithTvl, WithTvl } from '../../types';
import { BASES_TO_CHECK_TRADES_AGAINST } from '../../../constants';
import { getV3PoolSelectorConfig } from '../../utils/get-pool-selector-config';
...
function poolSelectorFactory<P extends WithTvl>({ getPoolSelectorConfig, getToken0, getToken1, getPoolAddress }: FactoryParams<P>) {
    return function tvlSelector(currencyA: Currency | undefined, currencyB: Currency | undefined, unorderedPoolsWithTvl: P[]) {
        ...
        const baseTokens: Token[] = (BASES_TO_CHECK_TRADES_AGAINST[chainId as ChainId] ?? []).map(currency => currency.wrapped);
        ...
        const topByBaseWithTokenIn = baseTokens
            .map(token => {
                return poolsFromSubgraph
                    .filter(subgraphPool => {
                        return (
                            (getToken0(subgraphPool).wrapped.equals(token) && getToken1(subgraphPool).wrapped.equals(currencyA.wrapped)) ||
            ...
            .map(token => {
                return poolsFromSubgraph
                    .filter(subgraphPool => {
                        ...
                        return (
                            (getToken0(subgraphPool).wrapped.equals(token) && getToken1(subgraphPool).wrapped.equals(currencyB.wrapped)) ||
        ...
        const top2DirectPools = poolsFromSubgraph
            .filter(subgraphPool => {
                ...
                return (
                    (getToken0(subgraphPool).wrapped.equals(currencyA.wrapped) && getToken1(subgraphPool).wrapped.equals(currencyB.wrapped)) ||
                    (getToken1(subgraphPool).wrapped.equals(currencyA.wrapped) && getToken0(subgraphPool).wrapped.equals(currencyB.wrapped))
                );
            })
        ...
        const nativeToken = Ether.onChain(chainId);
        const top2EthBaseTokenPool = nativeToken
            ? poolsFromSubgraph
                  .filter(subgraphPool => {
                      ...
                      return (
                          (getToken0(subgraphPool).wrapped.equals(nativeToken) && getToken1(subgraphPool).wrapped.equals(currencyA.wrapped)) ||
                          (getToken1(subgraphPool).wrapped.equals(nativeToken) && getToken0(subgraphPool).wrapped.equals(currencyA.wrapped))
                      );
                  })
                  .slice(0, 1)
            : [];
        ...
        const top2EthQuoteTokenPool = nativeToken
            ? poolsFromSubgraph
                  .filter(subgraphPool => {
                      ...
                      return (
                          (getToken0(subgraphPool).wrapped.equals(nativeToken) && getToken1(subgraphPool).wrapped.equals(currencyB.wrapped)) ||
                          (getToken1(subgraphPool).wrapped.equals(nativeToken) && getToken0(subgraphPool).wrapped.equals(currencyB.wrapped))
                      );
                  })
                  .slice(0, 1)
            : [];
