import { Currency, CurrencyAmount, ZERO } from '@real-wagmi/sdk';
import { Pool as V3Pool, TickList } from '@real-wagmi/v3-sdk';
import { Pool as IPool, Pool, QuoteProvider, QuoterOptions, RouteWithoutQuote, RouteWithQuote, V3Pool as IV3Pool } from '../types';
...
export function createOffChainQuoteProvider(): QuoteProvider {
    const createGetRoutesWithQuotes = (isExactIn = true) => {
        ...
        return async function getRoutesWithQuotes(routes: RouteWithoutQuote[], { gasModel }: QuoterOptions): Promise<RouteWithQuote[]> {
...
function createGetV3Quote(isExactIn = true) {
    return async function getV3Quote(pool: IV3Pool, amount: CurrencyAmount<Currency>): Promise<{ quote: CurrencyAmount<Currency>; numOfTicksCrossed: number } | null> {
