import { Currency, Token, ChainId } from '@real-wagmi/sdk';
import { getLlamaChainName } from '../../chains';
...
export function createCommonTokenPriceProvider<T = any>(getTokenPrices: GetTokenPrices<T>): CommonTokenPriceProvider<T> {
...
export const getTokenUsdPricesBySubgraph: GetTokenPrices<BySubgraphEssentials> = async ({ addresses, chainId, provider }) => {
    const client = provider?.({ chainId });
    if (!client) {
        throw new Error('No valid subgraph data provider');
    }
    const { bundle, tokens: tokenPrices } = await client.request<{ bundle: { ethPriceUSD: string }; tokens: { id: string; derivedETH: string }[] }>(tokenPriceQuery, {
        pageSize: 1000,
...
export const getCommonTokenPricesBySubgraph = createCommonTokenPriceProvider<BySubgraphEssentials>(getTokenUsdPricesBySubgraph);

type LlamaTokenPriceFetcherFactoryOptions = {
    endpoint: string;
};

const createGetTokenPriceFromLlmaWithCache = ({ endpoint }: LlamaTokenPriceFetcherFactoryOptions): GetTokenPrices<BySubgraphEssentials> => {
    ...
    return async ({ addresses, chainId }) => {
        ...
        const result: { coins?: { [key: string]: { price: string } } } = await fetch(`${endpoint}/${list}`).then(res => res.json());
...
export const getCommonTokenPrices = withFallback([
    ...
    {
        asyncFn: ({ currencyA, currencyB }: ParamsWithFallback) => getCommonTokenPricesByWalletApi({ currencyA, currencyB }),
        timeout: 3000,
    },
    {
        asyncFn: ({ currencyA, currencyB, v3SubgraphProvider }: ParamsWithFallback) => getCommonTokenPricesBySubgraph({ currencyA, currencyB, provider: v3SubgraphProvider }),
    },
]);
