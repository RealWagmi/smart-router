import { ChainId, Currency, CurrencyAmount, TradeType } from '@real-wagmi/sdk';
...
import { getPoolAddress, isV3Pool } from '../utils';
...
export function getBestSwapRouteBy(
    ...
    const estimatedGasUsedUSDs = bestSwap.map(routeWithValidQuote => {
        ...
        return CurrencyAmount.fromRawAmount(usdToken, routeWithValidQuote.gasCostInUSD.quotient * 10n ** BigInt(decimalsDiff));
    });
    ...
    if (!estimatedGasUsedUSD.currency.equals(gasCostL1USD.currency)) {
        ...
        estimatedGasUsedUSD = estimatedGasUsedUSD.add(CurrencyAmount.fromRawAmount(usdToken, gasCostL1USD.quotient * 10n ** BigInt(decimalsDiff)));
    } else {
...
const findFirstRouteNotUsingUsedPools = (usedRoutes: RouteWithQuote[], candidateRouteQuotes: RouteWithQuote[]): RouteWithQuote | null => {
