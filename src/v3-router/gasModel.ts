import { BigintIsh, Currency, CurrencyAmount, ChainId, tryParseAmount, Price } from '@real-wagmi/sdk';
import sum from 'lodash/sum.js';

import { BASE_SWAP_COST_V3, COST_PER_HOP_V3, COST_PER_INIT_TICK, COST_PER_UNINIT_TICK } from '../constants';
import { GasCost, GasEstimateRequiredInfo, GasModel, Pool, PoolProvider, PoolType, PriceReferences, RouteWithoutGasEstimate } from './types';
import { getNativeWrappedToken, getTokenPrice, getUsdGasToken, isV3Pool } from './utils';
...
export async function createGasModel({ gasPriceWei, poolProvider, quoteCurrency, blockNumber, quoteCurrencyUsdPrice, nativeCurrencyUsdPrice }: GasModelConfig): Promise<GasModel> {
    ...
    const priceInUsd = quoteCurrencyUsdPrice ? getTokenPriceByNumber(usdToken, quoteCurrency, quoteCurrencyUsdPrice) : undefined;
    const nativePriceInUsd = nativeCurrencyUsdPrice ? getTokenPriceByNumber(usdToken, nativeWrappedToken, nativeCurrencyUsdPrice) : undefined;
    const priceInNative = priceInUsd && nativePriceInUsd ? nativePriceInUsd.multiply(priceInUsd.invert()) : undefined;

    const estimateGasCost = ({ pools }: RouteWithoutGasEstimate, { initializedTickCrossedList }: GasEstimateRequiredInfo): GasCost => {
...
async function getHighestLiquidityNativePool(poolProvider: PoolProvider, currency: Currency, blockNumber?: BigintIsh): Promise<Pool | null> {
...
async function getHighestLiquidityUSDPool(poolProvider: PoolProvider, chainId: ChainId, blockNumber?: BigintIsh): Promise<Pool | null> {
    const usdToken = getUsdGasToken(chainId);
    const nativeWrappedToken = getNativeWrappedToken(chainId);
