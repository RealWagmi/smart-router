...
import { Currency, CurrencyAmount, Percent, TradeType, validateAndParseAddress, WETH9, ChainId } from '@real-wagmi/sdk';
import { FeeOptions, MethodParameters, Payments, PermitOptions, Position, SelfPermit, toHex } from '@real-wagmi/v3-sdk';
...
import { SmartRouterTrade, V3Pool, BaseRoute, RouteType } from '../types';
...
export abstract class SwapRouter {
     ...
     * @returns A string array of calldatas for the trade.
     */
    private static encodeMixedRouteSwap(trade: SmartRouterTrade<TradeType>, options: SwapOptions, routerMustCustody: boolean, performAggregatedSlippageCheck: boolean): Hex[] {
        let calldatas: Hex[] = [];
    ...
    public static swapAndAddCallParameters(
        ...
        const { calldatas, inputIsNative, outputIsNative, sampleTrade, totalAmountIn: totalAmountSwapped, quoteAmountOut, minimumAmountOut: minAmountOut } = SwapRouter.encodeSwaps(
            trades,
            options,
            true,
        );
        ...
        const tokenIn = inputIsNative ? WETH9[chainId as ChainId] : positionAmountIn.currency.wrapped;
        const tokenOut = outputIsNative ? WETH9[chainId as ChainId] : positionAmountOut.currency.wrapped;
